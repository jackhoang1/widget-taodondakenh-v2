      ...
      <div class="main">
        <div class="customer-info">
          ...
          <div class="setting-msg-area" :class="{ show: isShowSettingMsg}">
            ...
            <button
              v-if="isShowSettingMsg"
              class="btn btn-primary btn-small w-100 mb-4"
          ...
          <div class="form-row">
            ...
            <div class="col">
              <!-- <select
                v-model="city"
                v-on:change="handleDistrict();resetChangeCity()"
                class="form-control-sm rounded"
              >
              ...
              <SearchAddress
                class="rounded"
                :list_data_input="listCity"
                :placeholder="'Tỉnh/TP'"
                @data_output="city=$event"
                :getData="handleDistrict"
          ...
          <div class="form-row">
            <div class="col" style="margin-bottom:10px">
              <!-- <select
                v-model="district"
                v-on:change="handleWard();resetChangeDistrict()"
                class="form-control-sm rounded"
              >
                <option selected :value="{}">Quận/Huyện</option>
                <option
                  v-for="(district, ind) in listDistrict"
                  :value="district"
                  :key="ind"
                >{{district.name}}</option>
              </select>-->
              <SearchAddress
                :list_data_input="listDistrict"
          ...
          <div v-if="hasBranch">
            <select v-model="branch" class="form-control-sm rounded">
              ...
              <option
                v-for="(branch, ind) in listBranch"
                :value="branch"
                :key="ind"
        ...
        <div class="product">
          ...
          <div :class="['form-group', { 'show-note': isShowNote }]">
            ...
            <button
              ...
              class="btn btn-primary btn-small w-100 mb-4 rounded"
              :class="{'hide': !isShowNote}"
            >Done</button>
          </div>
          ...
          <div class="list-product">
            <div
              class="product-detail d-flex align-items-center border-bottom"
          ...
          <div class="order border-top">
            ...
            <button
              ...
              class="btn btn-primary mb-4 w-100 rounded"
            >Tạo đơn hàng</button>
          </div>
        ...
        <div :class="{ 'popup': true, 'show-popup': isShowPopup}">
          <div class="d-flex align-items-center" :class="{ 'hidden-search': !isShowPopup}">
            <div
              class="search rounded d-flex align-items-center flex-grow-1"
              :class="{ 'hidden-search': !isShowPopup}"
            >
              <img
                class="mx-2"
                src="https://img.icons8.com/ios-glyphs/48/000000/search.png"
                alt="icon"
              />
              <input
          ...
          <div v-show="isShowPopup" class="list-product">
            ...
            >
              ...
              <div class="flex-grow-1">
                ...
                >{{ item.product_name.substring(0, 60) + '...' }}</p>
            ...
            <div
              v-if="filterListProduct.length"
              class="product d-flex align-items-center border-bottom border-top py-2"
              v-for="(item, ind) in filterListProduct"
              :key="ind"
              @click="handleAddToCart(item)"
            >
              <img
        ...
        <div :class="[ 'order-info', { 'show-order-info': isShowOrderInfo }]">
          <div v-if="isShowOrderInfo">
            ...
            <p
              v-for="(item, ind) in cart"
              :key="ind"
            >{{`${item.product_quantity} ${item.product_name}`}}</p>
            <br />
...
export default {
  ...
  data() {
    return {
      ...
      msgContent:
        "Quý khách đã tạo thành công đơn hàng số {{order_id}} , đơn hàng của quý khách đang được giao vận.",
  ...
  methods: {
    handleCity() {
      console.log("this.platform_type", this.platformType);
      if (this.platformType == "HARAVAN") {
        console.log("1111111111111111111");
        return this.getListCityHaravan();
      }
      this.getListCityBotup();
    },
    handleDistrict() {
      if (this.platformType == "HARAVAN") {
        console.log("222222222222222222");
        return this.getListDistrictHaravan();
      }
    ...
    handleWard() {
      if (this.platformType == "HARAVAN") {
        ...
        return this.getListWardHaravan();
      }
      this.handleChooseDistrict();
    },
    formatNameWard(ward) {
      if ((this.platformType = "HARAVAN")) {
        this.ward.code = ward.ward_code;
        this.ward.name = ward.ward_name;
      }
    },
    resetAddress() {
      this.city = {};
    ...
    async getListCityHaravan() {
      try {
        let path =
          "https://chatbox-widget.botbanhang.vn/v1/widget-order-3rd/haravan/provinces";
        let params = {};
        let listCityHanavan = await Restful.get(path, params);
        console.log("listCityHanavan", listCityHanavan);
        if (listCityHanavan.data.code == 200 && listCityHanavan.data.data) {
          this.listCity = listCityHanavan.data.data;
        }
      } catch (e) {
        console.log(e);
      }
    },
    async getListDistrictHaravan() {
      try {
        let path =
          "https://chatbox-widget.botbanhang.vn/v1/widget-order-3rd/haravan/districts";
        let params = { province_id: this.city.id };

        let listDistrictHanavan = await Restful.get(path, params);

        console.log("listDistrictHanavan", listDistrictHanavan);
        if (
          listDistrictHanavan.data.code == 200 &&
          listDistrictHanavan.data.data
        ) {
          this.listDistrict = listDistrictHanavan.data.data;
        }
      } catch (e) {
        console.log(e);
      }
    },
    async getListWardHaravan() {
      try {
        let path =
          "https://chatbox-widget.botbanhang.vn/v1/widget-order-3rd/haravan/wards";
        let params = { district_code: this.district.code };

        let listWardHaravan = await Restful.get(path, params);

        console.log("listWardHaravan", listWardHaravan);
        if (listWardHaravan.data.code == 200 && listWardHaravan.data.data) {
          let list_ward = listWardHaravan.data.data;
          let convert_list_ward = list_ward.map((ward) => {
            return {
              name: ward.ward_name,
              code: ward.ward_code,
            };
          });
          console.log("convert_list_ward", convert_list_ward);
          this.listWard = convert_list_ward;
        }
      } catch (e) {
        console.log(e);
      }
    },
    async getListCityBotup() {
      try {
        let getListCity = await Restful.get(
          "https://ext.botup.io/v1/delivery/subvn/thongtin/tinhtp",
          {}
        );

        if (getListCity.data.data) {
    ...
    async handleSearch() {
      try {
        if (this.searchValue) {
          ...
          let searchValue = searchValueData;
          ...
          searchValue =
            searchValueData.charAt(0).toUpperCase() + searchValue.substring(1);
          params.search = searchValue;
          await this.getListProduct(params);
          if (this.listProduct.length) return;

          // Search với các searchValue viết thường
          searchValue = searchValueData.toLowerCase();
          params.search = searchValue;
          await this.getListProduct(params);
    ...
    async handleAddToCart(item) {
      try {
        ...
        if (
          postToCart.data &&
          postToCart.data.data &&
          postToCart.data.data.client_id
        ) {
    ...
    async callOrder(path, body) {
      try {
        ...
        if (
          ...
          postCreateOrder.data.data &&
          (postCreateOrder.data.data.snap_order ||
          ...
          if (postCreateOrder.data.data.order_id) {
            let content = this.covertMsgContent(
              postCreateOrder.data.data.order_id
            );
    ...
    async getListProduct(params) {
      try {
        ...
        if (
          getListProduct.data &&
          getListProduct.data.data[0] &&
          getListProduct.data.data[0].platform_type
        ) {
    ...
    async getInitialData() {
      ...
      await this.getListProduct({ access_token: this.appToken });
      this.handleCity();
...
.product .list-product .product-detail:hover {
...
.product .list-product .product-detail .edit img {
  ...
  height: 15px;
  cursor: pointer;
  filter: invert(24%) sepia(80%) saturate(4235%) hue-rotate(342deg)
...
.popup .search img {
  width: 17px;
  height: 17px;
  filter: invert(48%) sepia(0%) saturate(21%) hue-rotate(179deg)
...
.popup div > button img {
  width: 100%;
  height: 100%;
  filter: invert(30%) sepia(96%) saturate(2052%) hue-rotate(198deg)
